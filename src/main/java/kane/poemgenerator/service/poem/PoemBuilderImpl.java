...
public class PoemBuilderImpl implements PoemBuilder {
	...
	public void buildPoem(Rules rules) {
		try {
			...
			if (!poemRules.isEmpty()) {
				for (Entry<String, Rule> entry : poemRules.entrySet()) {
					generatePoem((PoemRule) entry.getValue(), poemRules, sb);
				}
			}
	...
	private void generatePoem(PoemRule poemRule, LinkedHashMap<String, Rule> poemRules, StringBuilder sb) throws Exception {
		try {
